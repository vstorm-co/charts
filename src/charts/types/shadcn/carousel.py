...
class CarouselItem(BaseComponent):
    ...
    content: str | BaseComponent = Field(..., description="Content of the carousel element - either a text or BaseComponent type object")
    
class CarouselConfig(CustomType):
    """A config for the Carousel component."""
    
    align: Optional[Literal["start"]] = "start"
    loop: Optional[Literal["true", "false"]] = "true"
    orientation: CarouselOrientation
    container_class: Optional[Literal["max-w-xs"]] = "max-w-xs"
